...
fn encode_capability(value: Capability) -> Result<fb::Capability, ProtocolError> {
    match value {
        ...
        Capability::SharedMemory => Ok(fb::Capability::SharedMemory),
        Capability::QueueLifecycle => Ok(fb::Capability::QueueLifecycle),
        Capability::QueueWriter => Ok(fb::Capability::QueueWriter),
        Capability::QueueReader => Ok(fb::Capability::QueueReader),
    }
}
...
fn decode_capability(value: fb::Capability) -> Result<Capability, ProtocolError> {
    match value {
        ...
        fb::Capability::SharedMemory => Ok(Capability::SharedMemory),
        fb::Capability::QueueLifecycle => Ok(Capability::QueueLifecycle),
        fb::Capability::QueueWriter => Ok(Capability::QueueWriter),
        fb::Capability::QueueReader => Ok(Capability::QueueReader),
