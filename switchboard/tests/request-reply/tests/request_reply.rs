...
fn build_remote_client(workspace_root: &Path) -> Result<PathBuf> {
    let module_root = workspace_root.parent().unwrap().join("remote-client");
...
fn build_atlas(workspace_root: &Path) -> Result<PathBuf> {
    let module_root = workspace_root.parent().unwrap().join("atlas");
...
fn build_switchboard(workspace_root: &Path) -> Result<PathBuf> {
    let module_root = workspace_root.parent().unwrap().join("switchboard");
...
fn runtime_binary_path(workspace_root: &Path) -> PathBuf {
    workspace_root
        .join("../../../selium/switchboard/target/debug")
...
async fn start_client(client: &Client) -> Result<Process> {
...
async fn request_reply_end_to_end() -> Result<()> {
    ...
    let client_process = start_client(&client).await.context("start client")?;
